...
%
% Expected input mat names from plot_contrast_effect.m:
%   <data_content>_<model_mode>_contrast_effect_<analysis_field>.mat
%
% Example:
%   raw_count_all_condition_model_contrast_effect_y.mat
%   raw_count_all_condition_model_contrast_effect_yRecon_use_all.mat
%   raw_count_condition_specific_models_contrast_effect_y.mat
%
% Metric names used in this comparison script:
%   dHL : high - low
%   CMI : (high - low) / (abs(high) + abs(low))
%
% Note:
...
%   dHL   % high - low, stored in result as delta_HL
%   CMI   % (high - low) / (abs(high) + abs(low))
%
% Examples:
%   metrics = {'CMI'};
...
% This prevents broken-axis figures from overwriting full-range figures.
...
%% ----------------------- Load all required contrast effect results -----------------------
...
for f = 1:numel(all_fields)
    ...
    result_file = build_contrast_effect_mat_file_local( ...
        io_dir, safe_data_content, model_mode, field_name);

    fprintf('\nLoading contrast effect result for field %s:\n  %s\n', ...
        field_name, result_file);

    result_map.(key) = load_contrast_effect_result_local(result_file, field_name);
end
...
for m = 1:numel(metrics)
    ...
    hfig = figure( ...
        ...
        'Name', sprintf('Contrast effect comparation: %s', metric_name), ...
    ...
    for b = 1:nBase
        ...
        for c = 1:nComp
            ...
            any_broken_axis_this_metric = any_broken_axis_this_metric || ...
                axis_info.use_broken_axis;

            % Diagonal reference line y = x.
            % For broken-axis display, draw the linear central segment and
            % compressed tail segments separately, so the skipped raw
            % interval is visibly broken.
    ...
    sgtitle(tl, sprintf('%s | %s | %s | neuron-wise contrast effect comparation', ...
    ...
    if ~isempty(valid_legend_handles)
        legend_names = group_names(valid_legend_mask);
...
function result_file = build_contrast_effect_mat_file_local( ...
    io_dir, safe_data_content, model_mode, field_name)

safe_field = sanitize_filename_local(field_name);

result_file = fullfile(io_dir, sprintf('%s_%s_contrast_effect_%s.mat', ...
    safe_data_content, model_mode, safe_field));

if ~isfile(result_file)
    error('Contrast effect result file not found:\n  %s', result_file);
end

end

function result = load_contrast_effect_result_local(result_file, expected_field)
...
function result_metric_field = metric_name_to_result_field_local(metric_name)

switch metric_name

    case 'dHL'
        result_metric_field = 'delta_HL';

    case 'CMI'
        result_metric_field = 'CMI';
