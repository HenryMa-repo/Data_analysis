...
%       .global_all
%       .global_by_group
%       .neuron_by_group%% data_reconstruction.m
% Post-hoc DLAG data reconstruction from already saved bestmodel*.mat files.
%
% Run this script after plot_dlag_results.m has saved the best-model mat file.
%
...
%
%   yRecon_use_*:
%       Reconstruction with d included:
%       d + selected latent contribution
%
%   yRecon_use_*_no_d:
...
%
% New fields for d-vs-latent decomposition:
% - seqEst(n).d stores the d-only reconstruction as yDim x T.
% - seqEst(n).yRecon_use_across_no_d stores only across latent contribution.
% - seqEst(n).yRecon_use_within_no_d stores only within latent contribution.
% - seqEst(n).yRecon_use_all_no_d stores all latent contribution.
...
for n = 1:numel(seqEst)
    ...
    % ---------------------------------------------------------------------
    % Latent-only reconstructions, without d
    % ---------------------------------------------------------------------
    ...
    % ---------------------------------------------------------------------
    % Original-style reconstructions, with d
    % ---------------------------------------------------------------------
    ...
    % ---------------------------------------------------------------------
    % Save d-only and base noiseless reconstructions
    % ---------------------------------------------------------------------
    seqEst(n).d = yBase;
    ...
    seqEst(n).yRecon_use_across_no_d = yRecon_use_across_no_d;
    seqEst(n).yRecon_use_within_no_d = yRecon_use_within_no_d;
    seqEst(n).yRecon_use_all_no_d = yRecon_use_all_no_d;
    ...
    % reconstructions within the same trial. This keeps comparisons among
    % the five noisy versions driven by the reconstruction signal, not by
    % different random noise draws.
    % ---------------------------------------------------------------------
    if add_R_noise_reconstruction
...
%
% The first four added specs are mostly diagnostic:
%   d_only
%   use_across_no_d
%   use_within_no_d
%   use_all_no_d
%
% They are included in reconstruction_R2.mat for completeness, even if the
% main use case is response-sum decomposition rather than R2 comparison.

base_specs = {
    'd_only', 'd';
    'use_across_no_d', 'yRecon_use_across_no_d';
    'use_within_no_d', 'yRecon_use_within_no_d';
    'use_all_no_d', 'yRecon_use_all_no_d';
