...
%       *_size_effect_<field><contrast>_wn0_2_st0_2_w01.mat
%       *_size_effect_<field><contrast>_wn0_2_st0_2_w02.mat
%   then this script compares fields within each time window separately.
%
% - If no time-window files exist and the old static file exists:
%       *_size_effect_<field><contrast>.mat
%   then this script loads the old file and behaves like the original
%   static comparison version.
%
% - Therefore, when plot_size_effect.m uses a full-trial single window and
%   saves old filenames, this script will automatically use those files.
%
...
%
% Must match the values used in plot_size_effect.m.
%
% If plot_size_effect.m generated multiple windows, this script will load:
%   _wn0_2_st0_2_w01
%   _wn0_2_st0_2_w02
% etc.
%
% If plot_size_effect.m generated a full-trial single window and saved the
% old filename, this script will load the old filename.
analysis_window = 0.2;
sliding_step    = 0.2;
...
% This is recommended for comparison scatter plots because y = x, x = 0,
% and y = 0 are reference structures.
...
for f = 1:numel(all_fields)
    ...
    field_files.(key) = locate_size_effect_files_for_field_local( ...
        ...
        contrast_filter.file_suffix, time_parameter_file_tag);
    ...
    for m = 1:numel(metrics)
        ...
        for b = 1:nBase
            ...
            for c = 1:nComp
                ...
                % For broken-axis display, draw the linear central segment
                % and compressed tail segments separately, so the skipped
                % raw interval is visibly broken.
        ...
        if ~isempty(valid_legend_handles)
            legend_names = group_names_this(isgraphics(legend_handles));
        ...
        output_suffix = [contrast_filter.file_suffix, window_suffix];
...
function entries = locate_size_effect_files_for_field_local( ...
    io_dir, safe_data_content, model_mode, field_name, ...
    contrast_file_suffix, time_parameter_file_tag)

safe_field = sanitize_filename_local(field_name);

base_name = sprintf('%s_%s_size_effect_%s%s', ...
...
% Fallback to the old static filename.
% This is expected when plot_size_effect.m generated a full-trial single
% window and therefore intentionally saved old filenames.
...
has_new_time_metadata = ...
    isfield(result, 'analysis_window') && ...
    isfield(result, 'sliding_step') && ...
    isfield(result, 'time_file_suffix');

if ~has_new_time_metadata
    if ~isempty(expected_window_suffix)
        warning(['Field %s was loaded from a time-window filename, but ', ...
            'time metadata is missing in the mat file. File:\n  %s'], ...
            expected_field, result_file);
    else
        fprintf(['Field %s appears to be an old static result or a full-trial ', ...
            'single-window result without time metadata.\n'], expected_field);
    end
    return;
end
...
% These fields may differ slightly by field or old file version.
...
if isempty(window_suffix)
    if nWindows == 1
        label = 'single window / old static result';
    else
