...
%      trials are pooled together. Other condition features, such as size,
%      direction, stim_name, etc., are not used for further subdivision.
...
if ~use_condition_specific_models
    ...
    for f = 1:nAnalysisFields
        ...
        field_cache(f).low_trial_response = trial_response(:, low_trial_mask);
        field_cache(f).high_trial_response = trial_response(:, high_trial_mask);
    ...
    for cc = 1:numel(condition_list)
        ...
        for f = 1:nAnalysisFields
            ...
            if this_condition_contrast_code == 1
                ...
                field_cache(f).low_trial_response = [ ...
                    field_cache(f).low_trial_response, trial_response]; %#ok<AGROW>
            ...
            elseif this_condition_contrast_code == 2
                ...
                field_cache(f).high_trial_response = [ ...
                    field_cache(f).high_trial_response, trial_response]; %#ok<AGROW>
            ...
            field_cache(f).model_source(cc).model_mode = model_mode; %#ok<AGROW>
...
for analysisFieldIdx = 1:nAnalysisFields
    ...
    contrast_effect_base_name = sprintf('%s_%s_contrast_effect_%s', ...
        safe_data_content, model_mode, safe_field);

    lowvshigh_fullrange_base_name = sprintf('%s_%s_lowvshigh_%s_fullrange', ...
        safe_data_content, model_mode, safe_field);

    lowvshigh_brokenaxis_base_name = sprintf('%s_%s_lowvshigh_%s_brokenaxis', ...
        safe_data_content, model_mode, safe_field);
    ...
    low_trial_response = field_cache(analysisFieldIdx).low_trial_response;
    high_trial_response = field_cache(analysisFieldIdx).high_trial_response;
    ...
    delta_HL(valid_delta_mask) = high_response(valid_delta_mask) - low_response(valid_delta_mask);
    CMI(valid_CMI_mask) = delta_HL(valid_CMI_mask) ./ CMI_denominator(valid_CMI_mask);
    ...
    contrast_effect_result.response_per_trial = 'sum across all time bins';
...
for g = 1:nGroups
    ...
    xlabel('Low contrast: total response per trial');
    ylabel('High contrast: total response per trial');
...
sgtitle(sprintf('%s, %s, seqEst.%s, contrast effect metrics', ...
    stim_tag, model_mode, analysis_field), ...
    'Interpreter', 'none');
